...
REM     """!
REM     ********************************************************************************
REM     @file  build_gui.bat
REM     @brief  Build script for WakeStation Shutdown Daemon GUI version
...
echo ============================================
echo WakeStation Shutdown Daemon GUI Builder
echo ============================================
if "%DEBUG_MODE%"=="true" (
...
if exist "..\src\helper\gen_version.py" (
    python ..\src\helper\gen_version.py . --type gui
    if errorlevel 1 (
...
REM GUI build keeps CLI_MODE = False (default GUI mode with tray support)
echo [INFO] Building in GUI mode (CLI_MODE = False)

REM No special GUI compile flag needed - CLI_MODE=False handles this

REM Prepare debug flags and hidden imports for PyInstaller
set PYINSTALLER_FLAGS=
set HIDDEN_IMPORTS=--hidden-import=bcrypt --hidden-import=cryptography --hidden-import=requests --hidden-import=dotenv --hidden-import=pystray --hidden-import=PIL --hidden-import=tkinter
...
if exist "shutdown_daemon_gui.spec" (
    echo [INFO] Building GUI executable using spec file...
    if exist "version_info.txt" (
        echo [INFO] Using generated version info file
    )
    if "%DEBUG_MODE%"=="true" (
        pyinstaller %PYINSTALLER_FLAGS% shutdown_daemon_gui.spec
    ) else (
    ...
    if exist "version_info.txt" (
        pyinstaller %PYINSTALLER_FLAGS% %HIDDEN_IMPORTS% --onefile --windowed --name shutdown_daemon-gui --icon=shutdown_daemon_icon.ico --version-file=version_info.txt shutdown_daemon.py
    ) else (
        pyinstaller %PYINSTALLER_FLAGS% %HIDDEN_IMPORTS% --onefile --windowed --name shutdown_daemon-gui --icon=shutdown_daemon_icon.ico shutdown_daemon.py
    )
)
...
set EXE_FOUND=false
if exist "dist\shutdown_daemon-gui.exe" (
    set EXE_FOUND=true
    set EXE_NAME=shutdown_daemon-gui.exe
    echo [INFO] Build successful - executable found: shutdown_daemon-gui.exe
...
if "%EXE_FOUND%"=="false" (
    ...
    echo [INFO] To get detailed debug logs, run: build_gui.bat --debug
    echo.
    ...
    if exist "build" (
        echo [INFO] Moving build logs to ..\buildlog\gui-failed-build\
        if not exist "..\buildlog\gui-failed-build" mkdir "..\buildlog\gui-failed-build"
        xcopy "build\*" "..\buildlog\gui-failed-build\" /E /I /Q >nul 2>&1
        rmdir /s /q "build"
    ...
    if exist "shutdown_daemon_gui.spec" (
        copy "shutdown_daemon_gui.spec" "..\buildlog\gui-failed-build\" >nul 2>&1
        echo [INFO] Copied shutdown_daemon_gui.spec to build log
    )
    ...
    if exist "version_info.txt" (
        copy "version_info.txt" "..\buildlog\gui-failed-build\" >nul 2>&1
        del "version_info.txt"
        echo [INFO] Removed version info file
    )

    echo [INFO] Debug information saved to: buildlog\gui-failed-build\
    echo.
    pause
    exit /b 1
)
...
if "%EXE_FOUND%"=="true" (
    echo [INFO] Moving %EXE_NAME% to project bin\Release\
    move "dist\%EXE_NAME%" "..\bin\Release\shutdown_daemon-gui.exe" >nul
    if errorlevel 1 (
        echo [WARNING] Failed to move executable to project bin\Release, keeping in dist folder
        set FINAL_LOCATION=dist\%EXE_NAME%
    ) else (
        echo [INFO] Executable moved to project bin\Release\
        set FINAL_LOCATION=..\bin\Release\shutdown_daemon-gui.exe
    )
) else (
    echo [WARNING] No executable found to move
)
...
if exist "..\bin\Release\shutdown_daemon-gui.exe" (
    echo.
    echo ============================================
    echo [SUCCESS] GUI BUILD COMPLETED!
    echo ============================================
    echo.
    echo Executable location: bin\Release\shutdown_daemon-gui.exe
    echo [INFO] File size:
    for %%A in ("..\bin\Release\shutdown_daemon-gui.exe") do echo         %%~zA bytes
    echo.
    echo [INFO] This GUI version includes:
    echo   - System tray integration
    echo   - Dry-run mode toggle
    echo   - Status monitoring
    echo   - Configuration dialogs
    echo.
    ...
    echo Executable location: shutdown-daemon\dist\%EXE_NAME%
    echo [INFO] File size:
    ...
    echo [DEBUG] Generated executable name was: %EXE_NAME%
) else (
    echo.
    echo ============================================
    ...
    echo [DEBUG] Expected executable name: shutdown_daemon-gui.exe
)
echo.
pause